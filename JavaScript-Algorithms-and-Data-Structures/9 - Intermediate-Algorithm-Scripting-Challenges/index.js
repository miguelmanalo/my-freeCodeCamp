...
function orbitalPeriod(arr) {
  ...
  const argCopy = arr.slice();
  const GM = 398600.4418;
  const earthRadius = 6367.4447;
  // iterate thru the array arg and assign each structure it's orbital period
  for (let i = 0, len = argCopy.length; i < len; i += 1) {
    const averageAlt = argCopy[i].avgAlt;
    const orbitalPeriodCalc = Math.round(
      2 * Math.PI * Math.sqrt((averageAlt + earthRadius) ** 3 / GM)
    );
    delete argCopy[i].avgAlt;
    argCopy[i].orbitalPeriod = orbitalPeriodCalc;
  }
  return argCopy;
}
